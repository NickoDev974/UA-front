...
export function updateProductStockInDatabase(productId, newQuantity) {
  ...
  return axios
    ...
    .then((res) => {
      console.log("Stock du produit mis à jour :", res.data);
      return res.data;
    })
    .catch((err) => {
      return err;
...
export function savePictureDataToDatabase(pictureData, file) {
  console.log("Données de l'image à enregistrer :", pictureData, file);
  ...
  console.log("image", file, "name", pictureData.name);
  return axios
...
export function getImagesByProductId(productId) {
  return axios
    ...
    .then((res) => {
      //console.log("api images rendues", res);
      return res.data.images; // Vous pouvez retourner directement les images ici
    })
    .catch((err) => {
      return err;
    });
