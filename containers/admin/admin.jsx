...
const Admin = (props) => {
  ...
  // Fonction mise a ours du status utilisateur
          ...
          displayProducts()
            .then((response) => {
              if (response.status === 200) {
                ...
                console.log("Avant l'appel à alert()");
                alert("Produit supprimer avec succés");
                console.log("apres l'appel à alert()");
              }
            })
            .catch((err) => console.log(err));
        }
    ...
    updateOneProduct(updatedProduct, productId)
      .then((res) => {
        if (res.status === 200) {
          ...
          console.log("Erreur lors de la mise à jour du statut du produit");
        }
      })
      .catch((err) => {
        console.log(
  ...
  const handleDeleteCategory = (categoryId) => {
    ...
    // Récupère tous les produits associés à la catégorie que tu souhaites supprimer
    const productsAssociated = product.products.filter(
      (product) => product.id_category === categoryId
    );

    // Vérifie s'il y a des produits associés
    if (productsAssociated.length === 0) {
      // Aucun produit n'est associé, tu peux supprimer la catégorie en toute sécurité
      deleteOneCategory(categoryId)
        .then((res) => {
          ...
          if (res.status === 200) {
            displaycategory()
              .then((response) => {
                console.log("Réponse de l'API pour les categories :", response);

                if (response.status === 200) {
                  // Mise à jour de localCategories avec les nouvelles données de catégorie
                  dispatch(loadCategory(response.result)); //setLocalCategories(response.result);
                }
              })
              .catch((err) => console.log(err));
          }
