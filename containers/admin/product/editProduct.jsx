...
const EditProduct = (props) => {
  // console.log("Props reçues dans EditProduct :", props);
  ...
  const saveCompleteProduct = async () => {
    ...
    //-----------------------------------------------------------cette partie presque fonctionnele-----------------------------------
    // // Ensuite, envoyez l'image supplémentaire vers le backend et enregistrez-la dans la table Picture
    // // console.log("Longueur de additionalImages :", additionalImages);
    // //console.log("Valeur de productId :", productId, props.params.id);
    // if (additionalImages.length > 0 && productId) {
    //   // console.log("coucou");
    //   const formData = new FormData();
    //   // console.log("Images supplémentaires à envoyer :", additionalImages);
    //   formData.append("image", additionalImages[0]);
    //   // console.log("nome de l'immage :", additionalImages[0].name);
    //   axios({
    //     method: "post",
    ...
    // setSuccessMessage(
    //   "La modification de votre produit a été enregistré avec succès !"
    // );

    //----------------------------------------------------------------------------------------------
    if (additionalImages.length > 0 && productId) {
      console.log("Images supplémentaires à enregistrer :", additionalImages);

      const promises = additionalImages.map(async (file) => {
        console.log("Image supplémentaire à enregistrer :", file);
        const formData = new FormData();
    ...
    <section className="addProduct">
      ...
      <form className="b-form" onSubmit={onSubmitForm}>
        ...
        {/* <input
          type="file"
          onChange={(e) => {
            setAdditionalImages([e.currentTarget.files[0]]);
          }}
        />
