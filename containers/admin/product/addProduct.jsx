...
const AddProduct = (props) => {
    ...
    addOneProduct(datas)
      .then((res) => {
        if (res.status === 200) {
          ...
          console.log("Erreur lors de l'ajout du produit:", res);
        }
      })
      .catch((err) => console.log(err));
  };
  ...
  const saveCompleteProduct = () => {
    console.log("enregistrement ", name);
    ...
    if (selectedFile === null) {
      ...
      axios({
        ...
        .then((res) => {
          console.log("coucou produit", res);
          if (res.data.status === 200) {
            ...
            console.log("verif id_product", productId);
            // Enregistrement des images supplémentaires dans la table Picture
            additionalImages.forEach((image) => {
              const formData = new FormData();
              formData.append("image", image);
              console.log(
                "Données de l'image supplémentaire à enregistrer :",
                formData
              );
              axios({
                method: "post",
  ...
  const onSubmitForm = (e) => {
    console.log("onsubmitform", name, price, content, stock);
    e.preventDefault();
    setError(null);
  ...
  return (
      ...
      <form className="productForm" onSubmit={onSubmitForm}>
        ...
        {/* <input
          ...
          onChange={(e) => {
            ...
            const renamedFiles = files.map((file) => {
              const newFileName = prompt(
                "Veuillez saisir un nouveau nom pour l'image :",
                file.name
              );
              console.log("Nouveau nom pour l'image :", newFileName);
              return new File([file], newFileName, { type: file.type });
        ...
        {/* ---------------------------code qui fonctionne mais pas de reel choix --------------------
        <select value={category} onChange={(e) => setCategory(e.target.value)}>
          <option value="4">Tous les produits</option>
        </select>
        <select
    ...
    ------------------------------------------------------------------------------------------ */}
        {/* <select value={category} onChange={(e) => setCategory(e.target.value)}>
          <option value="">Sélectionnez une catégorie</option>
          <option value="Alimentation poissons exotiques">
            Alimentation poissons exotiques
          </option>
          <option value="Alimentation KOI">Alimentation KOI</option>
          <option value="Produit entretien bassin">
            Produit entretien bassin
          </option>
